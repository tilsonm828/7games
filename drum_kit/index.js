...
//control the playback speed of record
function waitMoments() {
  return new Promise(resolve => setTimeout(resolve, 280)); // 1000 ms = 1 second
}
...
async function playRecord() {
  let keysRec = document.querySelector("#keys-pressed").innerHTML;
  console.log(keysRec);
  for (let i = 5; i < keysRec.length; i++) {
    console.log(keysRec[i]);
    makeSound(keysRec[i]);
    await waitMoments();
  } 
}
